...
public class ControllerRegPaciente implements Initializable {
	...
	void On_Login_Clicked(ActionEvent event) {
		...
		stageA.close();
		MedicoController.VentanaCuidador.show();
		/*
		 * try { Parent root =
	...
	void On_Register_Clicked(ActionEvent event) {
		...
		try {
			...
			if (fechaMin <= fechad.getYear() && fechad.getYear() <= fechaAct.getYear()
					&& fechad.getDayOfYear() <= fechaAct.getDayOfYear()) {
				...
				if (DatosP != null) {
					...
					TextError.setText(
							"The password must contain one upercase letter,\n a lowercase letter and a number");
					...
					Datos[9] = null;
					System.out.println(
							correo + Datos[0] + Datos[1] + Datos[2] + Datos[3] + Datos[4] + Datos[5] + Datos[6]);
