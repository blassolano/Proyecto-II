...
/*
		...
		Nombre.setText(Main1.ControladorLogin.PERSONA.getNombre() + " " + Main1.ControladorLogin.PERSONA.getApellidos());

		try {
			File dest = new File("src/Imagen/FotoPerfil" + Main1.ControladorLogin.PERSONA.getCorreo() + ".png");
			String thePath = dest.toURI().toURL().toExternalForm();
		    Image image = new Image(thePath);
		    Foto.setImage(image);
		    Main3.imagenPerfilControladorVentana = Foto;
		...
		String FamiliarDat = Main1.ControladorLogin.PAERSONA.getFamiliar().replace(" ","");

		Map<String, ClassPersona> mapFamily = Main1.Main3.leerArchivo();
		ClassPersona FamiliarDatos = mapFamily.get(FamiliarDat);
		

		//Datos Cuidador

		String Cuidador = Main1.ControladorLogin.PACIENTE.getCuidador().replace(" ","");
		Map<String, ClassPersona> map = Main1.Main3.leerArchivo();
		ClassPersona CuidadorDatos = map.get(Cuidador);
		NombreCuidador.setText(CuidadorDatos.getNombre());
		...
		FechaNacimiento.setText(CuidadorDatos.getEdad());
		try {
			String DireccionFotoP = "Imagen/FotoPerfil" + CuidadorDatos.getCorreo() + ".png";
			System.out.println(DireccionFotoP);
		    Image image = new Image(DireccionFotoP);
		    FotoCuidador.setImage(image);
		}catch(Exception e){
			e.printStackTrace();
		}
		//Datos Familiares
		...
		Map<String, String[]> mapC = Main1.Main3.leerArchivoClase();
		String[] p = mapC.get(Main1.ControladorLogin.PERSONA.getCorreo());
		String CorreoAnt = Correo1.getText();
		if(!p[2].equals("null")) {
			p[2] = p[2].replace(" ","");
			String[] Pacientes = p[2].split("\\|");

					Map<String, ClassPersona> mapF = Main1.Main3.leerArchivo();
					int l = Pacientes.length;
					System.out.println("Pacientes[0] = " + Pacientes [0]);
					System.out.println("Numero pacientes " + l);
					ClassPersona[] Paciente = new ClassPersona[l];
					for(int i = 0; i<l; i++) {
					...
					DNI1.setText(Paciente[0].getDNI());
					Correo1.setText(Paciente[0].getCorreo());
					FechaNacimiento1.setText(Paciente[0].getEdad());
					try {
		...
		try {
			...
			FileReader fr = new FileReader("sensores/" + Main1.ControladorLogin.PERSONA.getCorreo() + "-sensores.txt");
		...
		if(OnOfPulsometro.isSelected()) {
	    	try {
	    		String path = "sensores/" + Main1.ControladorLogin.PERSONA.getCorreo() + "-sensores.txt";
	    ...
	    menuItem1 = new MenuItem("Cuidador: " + CuidadorDatos.getCorreo() + " (" + CuidadorDatos.getNombre() + " " + CuidadorDatos.getApellidos() + ")");
	    menuItem1.setOnAction(new EventHandler<ActionEvent>() {
	    	@Override
	    	public void handle(ActionEvent event) {
	    	try {
	    	String path = "chat/" + Main1.ControladorLogin.PERSONA.getCorreo() + "-chat.txt";
			String content;
	    ...
	    if(!p[2].equals("null")) {
			p[2] = p[2].replace(" ","");
			String[] Familiares = p[2].split("\\|");
			ClassPersona[] Familiar = new ClassPersona[Familiares.length];
			for(int i = 0; i<Familiares.length; i++) {
				int r = i;
				Familiar[i] = map.get(Familiares[i]);
			    menuItem1 = new MenuItem("Familiar: " + Familiar[i].getCorreo() + " (" + Familiar[i].getNombre() + "" + Familiar[i].getApellidos()+ ")");
			    menuItem1.setOnAction(new EventHandler<ActionEvent>() {
			    	
			    	@Override
			    	public void handle(ActionEvent event) {
			    		CorreoChat = Familiar[r].getCorreo();
				    	try {
				    	String path = "chat/" + Familiar[r].getCorreo() + "-chat.txt";
						String content;
						content = readFile(path);
						chatStreaming.setText(content);
					}	catch (IOException e) {
		...
		// Historial medico
		try {
			String path = "historial/" + Main1.ControladorLogin.PERSONA.getCorreo() + "-historial.txt";
			String content;
			content = readFile(path);
	...
	public static Paciente DatosPaciente(String correo) {
		...
		Map<String, String[]> map = Main1.Main3.leerArchivoClase();
		String[] Datos = map.get(correo);
		ClassPaciente paciente = null;
		try {
			paciente = new ClassPaciente(Datos[1], Datos[2], Datos[3]);
	    	System.out.println("\tCuidador: " + paciente.getCuidador());
			System.out.println("\tFamiliar: " + paciente.getFamiliar());
			System.out.println("\tRuta: " + paciente.getHistorial());
		}catch(Exception e){
    ...
    void caidas_Actioned() {
    	...
    	try {
    		...
    		if(DetectorCaidas1.isSelected()) {
				...
				//infoCaidas.getChildren().add(contenido);
				try {
    ...
    void pulsometroActioned() {
    	...
    	try {
    		...
    		if(OnOfPulsometro.isSelected()) {
				...
				//HBox infoCaidas = new HBox();
				//infoPulsometro.getChildren().add(contenido);
				try {
		    		//Utilizo una lista para leer el fichero alreves
		    		FileReader fr = new FileReader(path);
		    		BufferedReader bf = new BufferedReader(fr);
    ...
    void On_Previous_Clicked(){
		...
		if(!p[2].equals("null")) {
			...
			Apellido1.setText(Paciente[numPac].getApellidos());
			NumTelefono1.setText(Paciente[numPac].getTelefono());
			DNI1.setText(Paciente[numPac].getDNI());
			Correo1.setText(Paciente[numPac].getCorreo());
			
			SimpleDateFormat sf = new SimpleDateFormat("dd/MM/yyyy");
			FechaNacimiento1.setText(sf.format(Paciente[numPac].getFecha()));
    ...
    void On_Next_Clicked(){
		...
		if(!p[2].equals("null")) {
			...
			for(int i = 0; i<l; i++) {
				//System.out.println("Datos paciente " + i + " " + map.get(Pacientes[i]));
				Paciente[i] = map.get(Pacientes[i]);
				//System.out.println("Paciente: " + Pacientes[i] + " Paciente anterior: " + CorreoAnt);
				if(CorreoAnt.equals(Pacientes[i])){
					numPac = i+1;
					//System.out.println("Numero Paciente " + numPac);
					if(numPac >= l) {
